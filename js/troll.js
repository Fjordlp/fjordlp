...
async function speak(text, lang) {
    ...
    // Варіант 1: власний Worker-проксі (/tts-api → VoiceRSS, безкоштовний
    // ключ без платіжної картки — код Worker'а див. у
    // worker-hardened-example.js). voiceCfg.edge тут лише як підказка мови
    // (Worker бере з нього BCP-47 префікс, напр. "nb-NO"). НЕ покриває
    // українську (uk) й ісландську (is) — для них і для будь-якої іншої
    // помилки цього маршруту (ключ не налаштований, вичерпано денну
    // квоту тощо) код нижче тихо піде у фолбек.
    if (typeof AI_PROXY_URL !== 'undefined' && AI_PROXY_URL) {
        try {
            ...
            if (res.ok) {
                ...
                // Спроба 1: Web Audio API (краще для якості й для деяких
                // мобільних браузерів, де прямий new Audio(blobUrl) інколи
                // не програється без явного audioContext.resume()).
                try {
                    ...
                    // Спроба 2: звичайний <audio> — потрібна як фолбек, бо
                    // Safari на iOS часто не вміє decodeAudioData() для MP3,
                    // хоча сам елемент <audio> той самий файл програє без проблем.
                    console.warn('Web Audio API не спрацював, пробуємо <audio>:', webAudioError);
