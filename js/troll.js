...
function trollLevelFromXp(xp) {
    let lvl = 1;
    for (let i = 0; i < TROLL_LEVEL_THRESHOLDS.length; i++) { if (xp >= TROLL_LEVEL_THRESHOLDS[i]) lvl = i + 1; }
    return lvl;
}
...
function checkAndApplyStreakFreeze() {
    ...
    if (dates.has(dayBefore) && !dates.has(yesterday) && !dates.has(today)) {
        ...
        updateState();
        const lang = (typeof STATE !== 'undefined' && STATE && STATE.uiLang) || 'uk';
        const usedMsg = { uk: 'Заморозку використано — серія', en: 'Freeze used — your streak of', ru: 'Заморозка использована — серия' }[lang];
        const savedMsg = { uk: 'врятована!', en: 'is saved!', ru: 'спасена!' }[lang];
...
function checkTrollUnlocks() {
    ...
    if (justUnlocked.length === 1) {
        const lang = (typeof STATE !== 'undefined' && STATE && STATE.uiLang) || 'uk';
        const unlockMsg = { uk: 'Тролль розблокував', en: 'Troll unlocked', ru: 'Тролль разблокировал' }[lang];
        toast(`${unlockMsg}: ${justUnlocked[0].emoji} ${gearItemName(justUnlocked[0])}!`);
    } else if (justUnlocked.length > 1) {
        const lang = (typeof STATE !== 'undefined' && STATE && STATE.uiLang) || 'uk';
        const unlockMsg = { uk: 'Розблоковано нове спорядження', en: 'New gear unlocked', ru: 'Разблокировано новое снаряжение' }[lang];
        toast(`${unlockMsg}: ${justUnlocked.length}${lang==='uk'?' шт.':''}!`);
    }
...
function addXP(amount, reason) {
    ...
    if (after > before) {
        const lang = (typeof STATE !== 'undefined' && STATE && STATE.uiLang) || 'uk';
        const msg = { uk: 'Рівень тролля підвищено', en: 'Troll level increased', ru: 'Уровень тролля повышен' }[lang];
        toast(`🧌 ${msg}: ${after}!`);
        if (after % 5 === 0) {
            if (typeof LD().streakFreezes !== 'number') LD().streakFreezes = 0;
            if (LD().streakFreezes < 3) {
                LD().streakFreezes++;
                const freezeMsg = { uk: 'Отримано заморозку серії! Тепер їх у вас', en: "Streak freeze earned! You now have", ru: 'Получена заморозка серии! Теперь у вас' }[lang];
...
const ACHIEVEMENTS = [
    { id: 'first_word', name: 'Перший крок', desc: 'Опрацюйте перше слово', check: s => Object.keys(s.stats.wordsSeen || {}).length >= 1 },
    { id: 'words_10', name: '10 слів', desc: 'Опрацюйте 10 слів', check: s => Object.keys(s.stats.wordsSeen || {}).length >= 10 },
    { id: 'words_50', name: '50 слів', desc: 'Опрацюйте 50 слів', check: s => Object.keys(s.stats.wordsSeen || {}).length >= 50 },
    { id: 'words_150', name: '150 слів', desc: 'Опрацюйте 150 слів', check: s => Object.keys(s.stats.wordsSeen || {}).length >= 150 },
    { id: 'streak_3', name: '3 дні поспіль', desc: 'Занімайтесь 3 дні поспіль', check: s => (s.streak || 0) >= 3 },
    ...
    { id: 'first_test', name: 'Перший тест', desc: 'Пройдіть перший тест', check: s => (s.stats.testsCompleted || 0) >= 1 },
    { id: 'tests_10', name: 'Десять тестів', desc: 'Пройдіть 10 тестів', check: s => (s.stats.testsCompleted || 0) >= 10 },
    { id: 'level_a2', name: 'Рівень A2', desc: 'Досягніть рівня A2', check: s => ['A2', 'B1', 'B2', 'C1', 'C2'].includes(s.level) },
    { id: 'level_b1', name: 'Рівень B1', desc: 'Досягніть рівня B1', check: s => ['B1', 'B2', 'C1', 'C2'].includes(s.level) },
    { id: 'level_c1', name: 'Рівень C1', desc: 'Досягніть рівня C1', check: s => ['C1', 'C2'].includes(s.level) },
    { id: 'perfect_test', name: 'Ідеальний результат', desc: '100% в одному тесті', check: (s, ctx) => !!(ctx && ctx.perfectTest) },
    { id: 'lessons_5', name: 'П\'ять уроків', desc: 'Завершіть 5 уроків', check: s => (s.lessonsDone || []).length >= 5 },
];
...
function checkAchievements(ctx) {
    ...
    ACHIEVEMENTS.forEach(a => {
        if (!LD().achievements.includes(a.id) && a.check(LD(), ctx)) {
            ...
            const lang = (typeof STATE !== 'undefined' && STATE && STATE.uiLang) || 'uk';
            const msg = { uk: 'Досягнення', en: 'Achievement', ru: 'Достижение' }[lang];
            toast(`🏆 ${msg}: ${achievementName(a)}!`);
...
async function speak(text, lang) {
    ...
    // Якщо є Worker – пробуємо отримати аудіо через нього
    if (typeof AI_PROXY_URL !== 'undefined' && AI_PROXY_URL) {
        try {
            ...
            console.warn('Worker TTS failed, using fallback:', e);
        }
    }
