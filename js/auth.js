...
function initAuth() {
    ...
    document.getElementById('guestBtn').onclick = () => {
        ...
        toast(t('auth_toast_guest_welcome'));
    };

    // ---- Скидання пароля ----
    document.getElementById('forgotPasswordLink').onclick = async () => {
        const email = prompt(t('auth_prompt_reset_email'));
        if (!email) return;

        try {
            await waitForFirebase(5000);
            if (!firebaseAuth) {
                toast(t('auth_toast_firebase_not_ready'));
                return;
            }
            await firebaseAuth.sendPasswordResetEmail(email);
            toast(t('auth_toast_reset_sent'));
        } catch (e) {
            let msg = t('auth_err_prefix');
            if (e.code === 'auth/user-not-found') {
                msg += t('auth_err_user_not_found');
            } else if (e.code === 'auth/invalid-email') {
                msg += t('auth_err_invalid_email');
            } else {
                msg += e.message;
            }
            toast(msg);
    ...
    submitBtn.onclick = async () => {
        ...
        if (!login || !password) {
            errorEl.textContent = t('auth_err_fill_both');
            return;
        }

        submitBtn.disabled = true;
        submitBtn.textContent = t('auth_btn_waiting');
        errorEl.textContent = '';

        try {
            await waitForFirebase(5000);
        } catch (e) {
            errorEl.textContent = t('auth_err_firebase_slow');
        ...
        if (!firebaseAuth) {
            errorEl.textContent = t('auth_err_auth_generic');
            submitBtn.disabled = false;
            submitBtn.textContent = isLogin ? t('submit_login') : t('submit_register');
            return;
        }
        ...
        if (isLogin) {
            ...
            if (!result.success) {
                errorEl.textContent = result.error || t('auth_err_invalid_credentials');
                submitBtn.disabled = false;
            ...
            checkAndApplyStreakFreeze();
            navigate(getInitialRoute());
            toast(tf('auth_toast_welcome', { name: displayName(STATE.name) || login }));
        } else {
            const result = await signUpWithFirebase(login, password);
            if (!result.success) {
                errorEl.textContent = result.error || t('auth_err_registration_generic');
                submitBtn.disabled = false;
                submitBtn.textContent = t('submit_register');
                return;
            }
            toast(t('auth_toast_registration_success'));
            switchMode();
            submitBtn.disabled = false;
            submitBtn.textContent = t('submit_login');
        }
