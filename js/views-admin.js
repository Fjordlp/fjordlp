    ...
    return `
        <div class="view">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                ...
                <div class="card admin-card" onclick="navigate('admin-sentence-gen')" style="cursor:pointer;transition:all .2s;">
                    <div style="font-size:2rem;margin-bottom:8px;">🧩</div>
                    <h3>Конструктор речень</h3>
                    <p style="color:var(--ink-soft);font-size:.85rem;">Згенерувати й опублікувати набір речень для вправи "склади з блоків"</p>
                </div>
...
// =====================================================================
//  СТОРІНКА: Конструктор речень (адмін генерує й публікує через AI)
// =====================================================================
function viewAdminSharedSentenceBuilder() {
    if (!STATE || !STATE.admin) return errorAccessDenied();
    return `
        <div class="view">
            <h1>🧩 Конструктор речень</h1>
            <p style="color:var(--ink-soft);margin-bottom:16px;">
                Згенеруйте набір речень для вправи "збери речення з блоків" — від
                найпростіших до складніших у межах рівня. Перегляньте прев'ю й
                опублікуйте — після цього набір одразу побачать усі користувачі цієї
                мови й рівня.
            </p>
            ...
            <div class="card" style="margin-bottom:16px;">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                    <div class="field">
                        <label>Мова</label>
                        <select id="sbLang">
                            ${LANGUAGES.filter(l => l.code !== 'no').map(l => `<option value="${l.code}">${l.flag} ${l.name.uk}</option>`).join('')}
                        </select>
                    </div>
                    <div class="field">
                        <label>Рівень</label>
                    ...
                    <div class="field" style="display:flex;align-items:flex-end;grid-column:1/-1;">
                        <button class="btn btn-primary btn-block" id="sbGenerateBtn">✨ Згенерувати</button>
                    </div>
                </div>
                <p id="sbStatus" style="color:var(--ink-soft);font-size:.85rem;margin-top:10px;"></p>
            </div>

            <div class="card" id="sbExistingCard" style="margin-bottom:16px;">
                <h3>📦 Уже опубліковано</h3>
                <div id="sbExistingList" style="color:var(--ink-soft);font-size:.85rem;">Завантаження…</div>
            </div>

            <div class="card" id="sbPreviewCard" style="display:none;">
                <h3>👀 Прев'ю (<span id="sbPreviewCount">0</span> речень)</h3>
                <div id="sbPreviewList" style="max-height:340px;overflow-y:auto;margin:12px 0;"></div>
                <button class="btn btn-primary" id="sbPublishBtn">📤 Опублікувати для всіх користувачів</button>
                <button class="btn btn-ghost" id="sbDiscardBtn">🗑️ Відхилити</button>
            </div>
...
function initAdminSharedSentenceBuilder() {
    const genBtn = document.getElementById('sbGenerateBtn');
    const statusEl = document.getElementById('sbStatus');
    const previewCard = document.getElementById('sbPreviewCard');
    const previewList = document.getElementById('sbPreviewList');
    const previewCount = document.getElementById('sbPreviewCount');
    const existingList = document.getElementById('sbExistingList');
    if (!genBtn) return; // не та сторінка

    let pendingSentences = null;
    let pendingLang = null;
    let pendingLevel = null;
    ...
    async function refreshExisting() {
        if (!firebaseReady || !firebaseDb) { existingList.textContent = 'Firestore недоступний.'; return; }
        try {
            const snap = await firebaseDb.collection('sharedSentenceBuilder').get();
            if (snap.empty) { existingList.textContent = 'Ще нічого не опубліковано.'; return; }
            const rows = [];
            snap.forEach(doc => {
                const d = doc.data();
                ...
                rows.push(`<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--line-soft);">
                    <span>${lang.flag} ${lang.name.uk} — <span class="tag level-${d.level}">${d.level}</span></span>
                    <span>${d.count || (d.sentences||[]).length} речень</span>
                </div>`);
            });
            existingList.innerHTML = rows.join('');
        } catch (e) {
            console.error('[Адмін] Помилка завантаження списку речень:', e);
            existingList.textContent = 'Помилка завантаження.';
        }
    }
    refreshExisting();
    ...
    genBtn.onclick = async () => {
        ...
        const level = document.getElementById('sbLevel').value;
        genBtn.disabled = true;
        const original = genBtn.textContent;
        statusEl.textContent = 'Генерую…';
        try {
            const sentences = await generateSentenceBuilderWithAI(lang, level);
            ...
            pendingLang = lang;
            pendingLevel = level;
            previewCount.textContent = sentences.length;
            previewList.innerHTML = sentences.map(s => `
                <div style="padding:8px 0;border-bottom:1px solid var(--line-soft);font-size:.85rem;">
                    <strong>${escHtml(s.no)}</strong> — ${escHtml(s.uk)}
                    <div style="color:var(--ink-soft);">${(s.blocks||[]).map(b => `[${escHtml(b)}]`).join(' ')}</div>
                </div>
            `).join('');
            previewCard.style.display = 'block';
            statusEl.textContent = `Готово: ${sentences.length} речень. Перевірте прев'ю нижче й опублікуйте.`;
        } catch (e) {
            console.error('[Адмін] Помилка генерації речень:', e);
            let reason = e && e.message || 'невідома помилка';
            if (e && e.code === 'PROXY_ERROR') {
                reason = `код ${e.status}` + (e.detail ? `: ${String(e.detail).slice(0, 200)}` : '');
                if (e.status === 429) reason += ' (забагато запитів підряд — зачекайте хвилину й спробуйте знову)';
    ...
    const publishBtn = document.getElementById('sbPublishBtn');
    if (publishBtn) publishBtn.onclick = async () => {
        if (!pendingSentences || !pendingSentences.length) return;
        publishBtn.disabled = true;
        try {
            await saveSharedSentenceBuilder(pendingLang, pendingLevel, pendingSentences);
            toast(`✅ Опубліковано ${pendingSentences.length} речень для ${getLanguage(pendingLang).name.uk} (${pendingLevel})`);
            previewCard.style.display = 'none';
            pendingSentences = null;
            refreshExisting();
        } catch (e) {
            console.error('[Адмін] Помилка публікації речень:', e);
            toast('⚠️ Не вдалося опублікувати. Перевірте доступ до Firestore.');
        } finally {
            publishBtn.disabled = false;
        }
    ...
    const discardBtn = document.getElementById('sbDiscardBtn');
    if (discardBtn) discardBtn.onclick = () => {
        pendingSentences = null;
        previewCard.style.display = 'none';
    };
