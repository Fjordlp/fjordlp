...
function viewAdminDaily() {
 ...
 Завдання публікується окремо для кожної мови навчання — люди, які вчать
 іншу мову, побачать своє (або взагалі нічого, якщо для їхньої мови на
 сьогодні ще нічого не опубліковано).
 </div>
 <div class="card">
 <h3> Створити завдання на сьогодні</h3>
 <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px;">
 <div class="field" style="grid-column:1/-1;">
...
window.adminToggleUserAdmin = async function(uid, makeAdmin) {
 ...
 toast(' Не вдалося змінити права. Перевірте правила доступу Firestore.');
 }
};
...
async function loadDailyList() {
 ...
 const langLabel = langInfo ? `${langInfo.flag} ${langInfo.name.uk}` : ' без мови (старий запис)';
 html += `
 <div class="card" style="margin-bottom:8px;">
...
function viewAdminSharedVocab() {
 ...
 <button class="btn btn-ghost" id="avDiscardBtn"> Відхилити</button>
 </div>
...
function initAdminSharedVocab() {
 ...
 statusEl.textContent = ` Помилка генерації: ${reason}`;
 } finally {
 ...
 toast(' Не вдалося опублікувати. Перевірте доступ до Firestore.');
 } finally {
...
function viewAdminSharedGrammar() {
 ...
 <button class="btn btn-ghost" id="agDiscardBtn"> Відхилити</button>
 </div>
...
function _abChapterEditorHtml(idx, chapter) {
 ...
 <button class="btn btn-ghost btn-sm ab-remove-chapter" data-idx="${idx}" type="button"> Прибрати</button>
 </div>
...
function initAdminBooks() {
 ...
 toast(' Спершу встав текст розділу (щонайменше кілька абзаців) — на його основі AI складе завдання.');
 return;
 }
 ...
 ch.tasks = tasks;
 statusSpan.textContent = tasks.length ? `✅ ${tasks.length} завдань` : ' AI не повернув завдань, спробуй ще раз';
 } catch (e) {
 ...
 statusSpan.textContent = ` Помилка: ${reason}`;
 } finally {
 btn.disabled = false;
 }
 };
 ...
 _abEditingBookId = id;
 document.getElementById('abFormTitle').textContent = ` Редагування: ${data.title}`;
 ...
 if (!title) { toast(' Введи назву книги.'); return; }
 if (!chapters.length || !chapters.some(c => c.text && c.text.trim().length > 0)) {
 toast(' Додай хоча б один розділ із текстом.');
 return;
 }
 if (!sourceNote) {
 toast(' Вкажи джерело/підтвердження, що це суспільне надбання — це важливо з юридичної точки зору.');
 return;
 }
 publishBtn.disabled = true;
 ...
 statusEl.textContent = ' Не вдалося опублікувати. Перевірте доступ до Firestore.';
 } finally {
 publishBtn.disabled = false;
 }
 };
...
function viewAdminDailyWord() {
 ...
 <div class="card">
 <h3> Опублікувати слово на сьогодні</h3>
 <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px;">
 <div class="field">
 <label>Мова навчання</label>
 <select id="dwLang">
