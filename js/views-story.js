...
function viewStory() {
    ...
    function renderLastChapter() {
        ...
        if (newWordsList && last.newWords) {
            last.newWords.forEach(w => {
                const shownTranslation = (STATE.vocabLang === 'en' && w.en) || (STATE.vocabLang === 'ru' && w.ru) || w.uk;
                const item = el(`<span style="display:inline-flex;align-items:center;gap:4px;"><strong>${escHtml(w.no)}</strong> — ${escHtml(shownTranslation)}</span>`);
    ...
    async function runChapter(choiceText) {
        ...
        try {
            ...
            let reason = e && e.message || t('story_error_unknown');
            if (e && e.code === 'PROXY_ERROR') {
                reason = e.status === 429 ? t('story_error_rate_limit') : tf('story_error_code', { status: e.status });
