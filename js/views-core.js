...
${(() => {
  ...
  return wrap;
        ...
        function viewVocabulary() {
            const level = STATE.level || "A1";
            const langObj = getLanguage(STATE.targetLang || 'no');
            ensureSharedVocabLoaded(STATE.targetLang || 'no', level); // тихо підвантажує спільний словник, якщо його ще нема в кеші
            const words = vocabForLevel(level);
            const statusLabels = {
            ...
            if (!langObj.builtin && words.length === 0) {
                // Звичайний користувач нічого не генерує — словник наповнює
                // адміністратор через адмін-панель (Спільні словники). Тут лише
                // дружнє повідомлення про очікування, і для адміна — ярлик.
                const emptyMsg = {
                    uk: `Словник для «${targetLangName()}», рівень ${level}, ще готується. Спробуйте пізніше або оберіть інший рівень.`,
                    en: `The vocabulary for "${targetLangName()}", level ${level}, isn't ready yet. Try again later or pick another level.`,
                    ru: `Словарь для «${targetLangName()}», уровень ${level}, ещё готовится. Попробуйте позже или выберите другой уровень.`,
                }[STATE.uiLang] || '';
                const wrap = el(`
                <div class="view">
                  <h1>${t('h_vocabulary')}</h1>
                  <div class="card" style="text-align:center;padding:40px 24px;">
                    <div style="font-size:2.4rem;margin-bottom:12px;">${langObj.flag}</div>
                    <p style="color:var(--ink-soft);margin-bottom:18px;">${emptyMsg}</p>
                    ${STATE.admin ? `<button class="btn btn-primary" id="goAdminVocabBtn">⚙️ Згенерувати в адмін-панелі</button>` : ''}
                  </div>
                </div>
              `);
                const goAdminBtn = wrap.querySelector('#goAdminVocabBtn');
                if (goAdminBtn) goAdminBtn.onclick = () => navigate('admin-vocab-gen', { presetLang: STATE.targetLang, presetLevel: level });
                return wrap;
            }
            const wrap = el(`
            <div class="view">
              ...
              <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px;">
                ...
                ${(langObj.builtin || STATE.admin) ? `<button class="btn btn-ghost btn-sm" id="vocabGenBtn" style="white-space:nowrap;">🎲 AI: додати нові слова</button>` : ''}
              </div>
          ...
          `);
            ...
            // AI-генерація нових слів: для норвезької додаються в STATE.customWords,
            // для інших мов — у STATE.generatedVocab[lang][level]. Обидва одразу
            ...
            if (genBtn) genBtn.onclick = async () => {
                ...
                try {
                    if (langObj.builtin) {
                        ...
                        // Для не вбудованих мов — той самий великий пакетний генератор,
                        // що й для стартового словника, але додає ПОВЕРХ уже наявних слів.
                        const more = await generateBulkVocab(STATE.targetLang, level, 3, (done, total) => {
                            genBtn.textContent = `🎲 ${done}/${total}`;
                        });
                        ensureGeneratedVocabStore();
                        const existingKeys = new Set(words.map(w => w.no.trim().toLowerCase()));
                        const uniqueNew = more.filter(w => !existingKeys.has(w.no.trim().toLowerCase()));
                        STATE.generatedVocab[STATE.targetLang][level] = words.concat(uniqueNew);
                        updateState();
                        toast(`✅ Додано нових слів: ${uniqueNew.length}`);
                        navigate('vocabulary');
                    }
                } catch (e) {
