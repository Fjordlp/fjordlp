    ...
    const wrap = el(`
        <div class="view">
            ...
            <div class="hero">
                ...
                <div class="eyebrow">${t('greeting_hi')}, ${escHtml(STATE.name && !isDefaultGuestName(STATE.name) ? STATE.name : t('student_word'))} · ${t('level_word')} <span class="tag level-${level}" style="vertical-align:middle;">${level}</span></div>
...
function renderWeekStrip() {
    ...
    // Раніше дні тижня в календарику стріку були захардкоджені українською
    // незалежно від мови інтерфейсу — людина з англійським/російським UI
    // все одно бачила "Нд Пн Вт...".
    const WEEKDAY_LABELS = {
        uk: ['Нд', 'Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб'],
        en: ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'],
        ru: ['Вс', 'Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб'],
    };
    const labels = WEEKDAY_LABELS[uiLang] || WEEKDAY_LABELS.uk;
...
function viewFlashDeckPicker(lang) {
    ...
    // Для перекладу назв тем зберігаємо перше слово кожної теми — якщо в
    // ньому є t_en/t_ru (AI генерує їх напряму разом зі словом), покажемо
    // саме їх замість евристичного словника тем.
    const topicSamples = {};
    words.forEach(w => { if (w && w.t && !(w.t in topicSamples)) topicSamples[w.t] = w; });
    ...
    [ALL_TOPICS_SENTINEL, ...topics].forEach(tp => {
        const label = tp === ALL_TOPICS_SENTINEL ? t('all_topics') : translateTopic(tp, null, topicSamples[tp]);
...
function viewFlashSession() {
    ...
    const progWrap = el(`
        <div class="flash-progress">
            <div class="meta"><span>${pos+1} / ${queue.length}</span><span>${escHtml(translateTopic(item.w.t, null, item.w))}</span></div>
        ...
        <div class="view">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px;">
                ...
                <select id="vocabTopic" class="chip" style="border-radius:999px;padding:6px 14px;font-size:.8rem;">
                    ...
                    ${[...new Set(words.map(w=>w.t))].map(tp=>`<option value="${escHtml(tp)}">${escHtml(translateTopic(tp, null, topicSamples[tp]))}</option>`).join('')}
                </select>
    ...
    function renderList() {
        ...
        tbody.innerHTML = filtered.map(w => {
            ...
            return `<tr>
                ...
                <td><span style="font-size:.7rem;background:var(--line-soft);padding:2px 8px;border-radius:999px;">${escHtml(translateTopic(w.t, null, w))}</span></td>
