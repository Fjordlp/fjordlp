...
// Раніше перевіряло ЛИШЕ прапорець STATE._onboardingDone — якщо він з
// якоїсь причини не був виставлений (наприклад, дані користувача
// імпортовані/відновлені вручну, або залишились зі старої, "плоскої"
// структури до переходу на langData), реальний, досвідчений користувач
// із роками прогресу міг знову побачити онбординг для новачків. Тепер
// додатково перевіряє, чи є в акаунті РЕАЛЬНІ ознаки прогресу (XP,
// вивчені слова, пройдені тести, власні слова, SRS-дані) по будь-якій
// мові — і якщо так, вважає онбординг завершеним автоматично.
...
// =====================================================================
//  ІСТОРІЯ ТЕСТІВ — збереження в localStorage (щоб не перевищувати 1 МБ
//  Firestore) — у LD().testHistory (Firestore/synced state) лежать лише
//  метадані кожного тесту (тип/рівень/дата/бал), а повні деталі
//  (питання/відповіді користувача/правильні відповіді) — окремо в
//  localStorage за ключем testHistory[i].id. Той самий принцип, що й
//  обмеження story.history чи booksProgress: не роздувати документ, що
//  синхронізується цілком при кожній дії.
// =====================================================================
...
function addTestResult(testResult) {
    ...
    // У синхронізований стан (Firestore) зберігаємо ТІЛЬКИ метадані —
    // details навмисно відсутні, вони лишаються локально.
    const metaOnly = {
        id: testResult.id,
...
// Записує щойно завершений тест в історію — спільна логіка для всіх 5
// типів тестів (test-mc, test-listen — через runQuiz; test-cloze,
// test-order, test-translate — кожен окремо). Ідемпотентна (sub.
// _historyRecorded), бо екран завершення тесту може перемальовуватись
// кілька разів після завершення (той самий принцип, що й sub._xpGranted
// для нарахування XP) — без цього прапорця один тест дублювався б в
// історії щоразу.
function recordTestHistoryIfNeeded(sub, route, pct) {
    if (sub._historyRecorded) return;
    sub._historyRecorded = true;
    const details = sub.qs.map((q, idx) => {
        const ans = sub.userAnswers[idx];
        const isCorrect = !!(ans && ans.correct === true);
        const questionText = q.q || q.sentence || q.translation || '';
        // Якщо правильна відповідь уже порахована самим тестом (як у
        // test-translate — ans.correctAnswer), довіряємо їй; інакше
        // виводимо з форми питання.
        let correctAnswer = (ans && ans.correctAnswer) || '';
        if (!correctAnswer) {
            if (q.opts && q.a !== undefined) correctAnswer = q.opts[q.a] || '';
            else if (q.original) correctAnswer = q.original.join(' ');
        }
        let userAnswer = '';
        if (ans) {
            if (ans.input !== undefined) userAnswer = ans.input;
            else if (ans.chosen && ans.shuffled) {
                // test-order: chosen — індекси у shuffled, а не в original
                userAnswer = ans.chosen.map(i => ans.shuffled[i] || '').join(' ');
        ...
        return { question: questionText, userAnswer, correctAnswer, isCorrect };
    });
    addTestResult({
